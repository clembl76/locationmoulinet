...
- `app/admin/layout.tsx` : ajout de la police Plus Jakarta Sans (`lib/fonts.ts`, déjà créé en phase 1) sur un seul point d'insertion englobant toutes les pages admin
- `components/admin/AdminNavbar.tsx` : recoloré + ajout de la mise en évidence du lien actif (`usePathname()`, soulignement teal) — ordre des liens de navigation inchangé
- Fiche appartement admin (`app/admin/apartments/[number]/page.tsx`) : glyphes Unicode `✓`/`✗` (indicateurs Encaissé/Non encaissé) remplacés par des icônes SVG au trait
- `components/admin/EdlFigeView.tsx` : bascule Entrée/Sortie et bouton "Générer le pdf" recolorés (`bg-gray-900`) ; `components/admin/AvailabilityManager.tsx` : interrupteurs (visites/candidatures) recolorés en pilule teal quand actifs
- **Conservé volontairement inchangé** : tous les badges de statut multi-couleurs existants (vert/ambre/rouge/violet — disponible/départ/loué, etc., dans `MoisLoyersClient.tsx`, `DashboardAnnualClient.tsx`, `ApartmentsClient.tsx`, `LettingTable.tsx`, `VisitsTable.tsx`, `AdminActionsTable.tsx`) : ce sont des couleurs de catégorisation de données, pas des couleurs de marque — seule l'entrée "Moulinet" de `LinxoTable.tsx` (qui réutilisait la teinte bleue de marque) est passée en teal, les 3 autres bâtiments (emerald/amber/violet) restent inchangés
- Tests : `AdminNavbar.test.tsx` (3 tests lien actif ajoutés), `EdlFigeView.test.tsx` (6 assertions de classe `bg-blue-primary` → `bg-gray-900`) ; nouveaux fichiers de test colocalisés (composants jusque-là non testés) : `StatCard`, `GenerateRentsButton`, `ExportLeasesButton`, `ApartmentsClient`, `QuittanceButton`, `DepositPaidCheckbox`, `InsuranceCheckbox`, `AttestationLocationButton`, `DocusignUrls`, `PreavisButton`, `SendTenantListButton`, `LettingTable`, `AvailabilityManager`, `QuittancesGenerator`, `QuittanceCautionButton`
- `npm run test:coverage` (657 tests, 67 fichiers, tous verts) puis `npm run build` exécutés avant livraison

### 2026-09-12 — Fix : la page d'accueil ne correspondait pas à la maquette validée
- **Retour utilisateur** : après la refonte ci-dessous, la home ne ressemblait pas à la maquette `Home.dc.html` — filtres différents, halos de couleur absents, cartes d'appartement d'un autre design
- `components/HomeClient.tsx` : ajout des deux halos dégradés (teal + chaud) derrière le hero (`radial-gradient`, absolus, `overflow-hidden` scopé au hero uniquement pour ne pas casser le `sticky` du Navbar) ; `FilterBar` simplifiée conforme à la maquette — pilules de statut monochromes (actif = noir plein, inactif = blanc contouré, au lieu des couleurs vert/ambre/rouge), sans carte englobante ni libellé "Statut", réglettes surface/prix intégrées dans des pilules compactes en bout de ligne
- `components/ApartmentCard.tsx` : carte reconstruite sur le modèle de la maquette — plus de carte blanche à bordure/ombre autour de la photo, photo en `aspect-[4/3]` avec le badge de statut flottant en bas à gauche (monochrome : blanc/noir selon disponible ou loué, comme sur la maquette), titre "Appartement {n}" en gras suivi de la ligne "{type} · {surface} m² · {étage}", prix sans séparateur au-dessus ; galerie de la carte simplifiée à la photo de couverture (la bande de vignettes cliquables reste sur la fiche appartement, absente de la maquette de la grille)
- Tests : `ApartmentCard.test.tsx` mis à jour (nouvelle structure de texte fusionnée type/surface/étage)
- `npm run test:coverage` (557 tests) puis `npm run build` exécutés avant livraison

### 2026-09-12 — Refonte visuelle des pages publiques (accueil, fiche appartement, candidater, visiter)
- **Contexte** : nouvelle direction visuelle explorée avec le skill de design Claude (voir `design/homepage-refresh-2026-09/README.md`) — tons clairs, police Plus Jakarta Sans, accent teal unique, boutons/badges en pilule — implémentée uniquement sur les pages publiques. `/admin*` volontairement non touché (phase 2 séparée) : nouveau token `--color-teal` ajouté à côté des tokens `blue-*` existants (jamais retirés), nouvelle police chargée via `lib/fonts.ts` et appliquée uniquement sur un wrapper par page publique (`app/page.tsx`, `app/apartments/[number]/page.tsx`, `app/candidater/page.tsx`, `app/visiter/page.tsx`) — `app/layout.tsx` et `/admin` gardent Geist et le bleu intacts
- `components/Navbar.tsx`, `LanguageSwitch.tsx` : recolorés (accent teal, CTA en pilule noire)
- `components/HomeClient.tsx`, `ApartmentCard.tsx` : hero et cartes recolorés ; `FilterBar` (pilules de statut + sliders de surface/prix) réhabillée sans changement de logique ; pied de page passé d'un bandeau bleu foncé à un pied de page clair
- `components/ApartmentDetail.tsx` : restructuration en une seule colonne (specs à plat, bloc "Loyer mensuel" compact avec CTA `Nous contacter`/`Visiter` intégrés, "Conditions de location", puis "Quartier & commodités" déplacé en dernière section et transformé en accordéon `useState`) ; suppression des constantes mortes `CONDITIONS_FR`/`CONDITIONS_EN` (jamais utilisées)
- `components/CandidateForm.tsx`, `VisitorForm.tsx` : recoloration complète de tous les champs/boutons/cartes, **aucun changement de logique, de validation ni d'ordre des champs** ; états vides ("candidature suspendue", `app/candidater/page.tsx` et `app/visiter/page.tsx`) recolorés, emoji 🗓️ remplacé par une icône SVG au trait
- Tests : `Navbar.test.tsx` inchangé (assertions par rôle/texte, toujours valides) ; `CandidateForm.test.tsx` — une assertion de classe (`border-blue-primary` → `border-teal`) mise à jour ; nouveaux fichiers de test colocalisés (composants jusque-là non testés) : `HomeClient.test.tsx`, `ApartmentCard.test.tsx`, `ApartmentDetail.test.tsx`, `VisitorForm.test.tsx`, `LanguageSwitch.test.tsx`
- `npm run test:coverage` (557 tests, 52 fichiers, tous verts) puis `npm run build` exécutés avant livraison
