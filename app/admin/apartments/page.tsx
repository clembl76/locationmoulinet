import { getAdminApartments, getFutureLeases, getClosingLeases, getArchivedLeases } from '@/lib/adminData'
...
export default async function AdminApartmentsPage({
  ...
  const [apartments, futureLeases, closingLeases, archivedLeases] = await Promise.all([
    getAdminApartments(),
    getFutureLeases(),
    getClosingLeases(),
    getArchivedLeases(),
  ...
  return (
    <div className="space-y-6">
      ...
      {closingLeases.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <h2 className="px-5 pt-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Baux en cours de clôture</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-xs text-gray-400 uppercase tracking-wider">
                  <th className="text-left px-5 py-3 font-semibold whitespace-nowrap">Appt</th>
                  <th className="text-left px-5 py-3 font-semibold">Locataire</th>
                  <th className="text-left px-5 py-3 font-semibold whitespace-nowrap">Départ</th>
                  <th className="text-right px-5 py-3 font-semibold whitespace-nowrap hidden sm:table-cell">Loyer CC</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {closingLeases.map((cl) => (
                  <tr
                    key={cl.lease_id}
                    className="hover:bg-gray-50 transition-colors"
                  >
                    ...
                    <td className="px-5 py-3 text-gray-700">
                      <a href={`/admin/apartments/${cl.apartment_number}?lease=${cl.lease_id}`} className="block">
                        {[cl.tenant_first_name, cl.tenant_last_name].filter(Boolean).join(' ') || '—'}
                      </a>
                    </td>
                    <td className="px-5 py-3 text-gray-700 whitespace-nowrap">
                      <a href={`/admin/apartments/${cl.apartment_number}?lease=${cl.lease_id}`} className="block">
                        {new Date(cl.move_out_date + 'T12:00:00').toLocaleDateString('fr-FR')}
                      </a>
                    </td>
                    <td className="px-5 py-3 text-right text-gray-500 whitespace-nowrap hidden sm:table-cell">
                      <a href={`/admin/apartments/${cl.apartment_number}?lease=${cl.lease_id}`} className="block">
                        {cl.rent_including_charges != null ? `${cl.rent_including_charges} €` : '—'}
                      </a>
                    </td>
                  </tr>
                ))}
      ...
      <ArchivesSection archivedLeases={archivedLeases} />
    </div>
  )
}
