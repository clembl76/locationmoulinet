...
vi.mock('@/app/admin/apartments/[number]/actions', () => ({
  updateEdlSentAction: (...args: unknown[]) => mockUpdate(...args),
}))
...
describe('EdlSentCheckbox', () => {
  it('affiche la case décochée quand initialValue=false', () => {
    render(<EdlSentCheckbox leaseId="l1" aptNumber="7" initialValue={false} />)
    expect(screen.getByRole('checkbox')).not.toBeChecked()
  })

  it('affiche la case cochée quand initialValue=true', () => {
    render(<EdlSentCheckbox leaseId="l1" aptNumber="7" initialValue={true} />)
    expect(screen.getByRole('checkbox')).toBeChecked()
  })

  it('coche la case et appelle updateEdlSentAction(leaseId, aptNumber, true)', async () => {
    mockUpdate.mockResolvedValue({ ok: true })
    const user = userEvent.setup()
    render(<EdlSentCheckbox leaseId="lease-1" aptNumber="7" initialValue={false} />)
